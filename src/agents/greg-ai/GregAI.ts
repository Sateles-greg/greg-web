    ...
    messages: [
      {
        role: 'system',
        content:
          'Você é Greg, uma IA clínica que faz triagem baseada no histórico pessoal do usuário.',
      },
      {
        role: 'user',
  ...
  const functions = [
    {
      ...
      description:
        'Simula 3 consequências possíveis para uma decisão simbiótica',
      parameters: {
        type: 'object',
        properties: {
          acao: {
    ...
    messages: [
      {
        role: 'system',
        content:
          'Você é Greg, uma I.A. simbiótica estrategista, focada em proteger e aprimorar a vida do usuário.',
      },
...
export async function analisarSentimento(texto: string) {
  const payload = {
    ...
    messages: [
      ...
      {
        role: 'user',
        content: `Analise o sentimento da seguinte frase: ${texto}`,
      },
    ],
  };
